...
const subscribeMessage = async (channel: amqp.Channel, service: typeof ShoppingService  ) =>{
    console.log("START CONSUME");
    const appQueue = await channel.assertQueue(config.app.QUEUE_NAME)
    channel.bindQueue(appQueue.queue, EXCHANGE_NAME, config.app.SHOPPING_BINDING_KEY)
